...
const EditTodaysStuff = () => {
  ...
  useEffect(() => {
    const fetchTodayFields = async () => {
      try {
        const res = await fetch("https://cop4331group3.xyz/api/activities/retrievetodayeditfields", {
          method: "POST",
        ...
        console.error("Failed to load today's fields:", err);
        setError("Failed to load today's minutes.");
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://cop4331group3.xyz/api/activities/editcategories", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          UserID: userID,
      ...
      setSuccess("Successfully updated categories!");
...
export default EditTodaysStuff;
